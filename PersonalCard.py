...
class PersonalCard:
    def __init__(self,
        ...
        if save_extract_result == True:
            if path_to_save == None or path_to_save == "":
                raise ValueError(
                    "Please define your path to save extracted images.")
    ...
    def __loadSIFT(self):
        self.source_image_front_tempalte = self.__readImage(
            os.path.join('datasets', 'identity_card/personal-card-template.jpg'))
        self.source_image_back_tempalte = self.__readImage(os.path.join('datasets', 'identity_card/personal-card-back-template.jpg'))
        ...
        with open(os.path.join('datasets', 'identity_card/config.yaml'), 'r') as f:
            try:
    ...
    def __findAndWrapObject(self, side: Card = Card.FRONT_TEMPLATE):
        ...
        if self.save_extract_result:
            cv2.imwrite(os.path.join(self.path_to_save,
                        'image_scan.jpg'), self.image_scan)
    ...
    def __extractItems(self, side: Card = Card.FRONT_TEMPLATE):
        for index, box in enumerate(
                self.roi_extract["roi_extract"][str(side)] if str(self.lang) == str(Language.MIX) else filter(
                    lambda item: str(self.lang) in ["eng", "tha"],
                    self.roi_extract["roi_extract"])):
            box = self.roi_extract["roi_extract"][str(side)][index]
            ...
            if str(self.provider) == Provider.DEFAULT.value:
                ...
                custom_config = r'-l tha --oem 3 --psm 6'
                self.cardInfo[str(self.lang)][box["name"]] = str.strip(
                    " ".join(pytesseract.image_to_string(imgCrop, lang=box["lang"].split(",")[0],
        ...
        if str(self.lang) == str(Language.MIX) and str(side) == str(Card.FRONT_TEMPLATE):
            ...
            self.cardInfo[str(self.lang)]["PrefixTH"] = str(
                "".join(extract_th[0]))
            self.cardInfo[str(self.lang)]["NameTH"] = str(
                "".join(extract_th[1] if len(extract_th) > 2 else extract_th[-1]))
            self.cardInfo[str(self.lang)]["LastNameTH"] = str(
                "".join(extract_th[-1]))

            extract_en = self.cardInfo[str(self.lang)]["NameEN"].split(' ')
            self.cardInfo[str(self.lang)]["PrefixEN"] = str(
                "".join(extract_en[0]))
            self.cardInfo[str(self.lang)]["NameEN"] = str(
                "".join(extract_en[1:]))
        ...
        if str(side) == str(Card.BACK_TEMPLATE):
            self.cardInfo[str(self.lang)]["LaserCode"] = "".join(re.findall(
                "([a-zA-Z0-9])", self.cardInfo[str(self.lang)]["LaserCode"])).upper()

        _card = namedtuple('Card', self.cardInfo[str(self.lang)].keys())(
            *self.cardInfo[str(self.lang)].values())
        return _card
    ...
    def extract_back_info(self, image):
        ...
        self.process_kp, self.process_des = self.sift.detectAndCompute(
            self.image, None)
        self.__compareTemplateSimilarity(
            self.process_des, self.source_back_des)
