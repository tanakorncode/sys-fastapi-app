...
class DrivingLicense:
    ...
    def __loadSIFT(self):
        self.source_image_front_tempalte = self.__readImage(
            os.path.join('datasets', 'driving_license/thai-driving-license-template.jpg'))
        self.source_front_kp, self.source_front_des = self.sift.detectAndCompute(
            self.source_image_front_tempalte, None)
        with open(os.path.join('datasets', 'driving_license/config.yaml'), 'r') as f:
            try:
        ...
        try:
            try:
                ...
                img = cv2.imdecode(np.fromstring(base64.b64decode(
                    image, validate=True), np.uint8), cv2.IMREAD_GRAYSCALE)
        ...
        for index, box in enumerate(
            ...
            if str(self.provider) == Provider.DEFAULT.value:
                if str(box["provider"]) == str(str(Provider.EASYOCR)):
                    self.cardInfo[str(self.lang)][box["name"]] = " ".join(str.strip("".join(self.reader.readtext(
                        imgCrop, detail=0, paragraph=True, width_ths=1.0, blocklist=box["blocklist"]))).split())
